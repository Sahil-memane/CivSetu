...
const { notifyAdminOnBreach } = require("./notificationService");
...
function getUpdatedSLAStatus(issue) {
    if (!issue.slaEndDate || issue.status === "resolved" || issue.status === "rejected") {
    ...
    const daysRemaining = parseFloat((timeRemaining / (1000 * 60 * 60 * 24)).toFixed(2));
    ...
    try {
        ...
        snapshot.forEach((doc) => {
            ...
            if (updates) {
                ...
                if (updates.slaStatus === "BREACHED" && issue.slaStatus !== "BREACHED") {
                    breachedIssues.push({ id: doc.id, ...issue, priority: issue.priority || "MEDIUM" });
                }
