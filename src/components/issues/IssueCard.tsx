...
export function IssueCard({ issue, onClick }: IssueCardProps) {
  return (
    <div
      onClick={onClick}
      className="group bg-card rounded-2xl border border-border/50 overflow-hidden hover:border-primary/30 hover:shadow-card-hover transition-all duration-300 cursor-pointer"
    >
      {/* Image */}
      {issue.imageUrl && (
        <div className="h-40 bg-muted relative overflow-hidden">
          ...
          <div className="absolute top-3 left-3 flex gap-2">
            <Badge className={cn("text-xs font-medium", priorityConfig[issue.priority].class)}>
              {issue.priority === "critical" && <AlertTriangle className="w-3 h-3 mr-1" />}
      ...
      <div className="p-5">
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <span className="text-lg">{categoryIcons[issue.category] || categoryIcons.other}</span>
            <span className="text-sm font-medium text-muted-foreground capitalize">{issue.category}</span>
          </div>
